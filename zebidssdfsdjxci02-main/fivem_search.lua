...
local function DrawPlaidUI()
    ...
    local uiWidth, uiHeight = 0.25, 0.50
    ...
    for i, tab in ipairs(tabs) do
        ...
        -- Click en tab
        if IsMouseInBounds(tabX, tabY, tabWidth, 0.04) and IsControlJustPressed(0, 237) then
            selectedTab = i
        end
    end
    ...
    if selectedTab == 1 then
        ...
        for i, button in ipairs(buttons) do
            ...
            -- Click en botón
            if isHovered and IsControlJustPressed(0, 237) then -- Click izquierdo
    ...
    local date = os.date("Plaid | %d/%m | Last Updated: 1d ago")
    DrawTextUI(date, baseX + 0.01, footerY + 0.007, 0.28, 4, 150, 150, 150, 255, false)
