...
local function IsMouseOverButton(x, y, w, h)
    if not IsNuiFocused() then
        local success, cursorX, cursorY = pcall(GetNuiCursorPosition)
        if success and cursorX and cursorY then
...
local function DrawUI()
    ...
    for i, option in ipairs(options) do
        ...
        -- Click en toggle (simplificado para evitar errores)
...
Citizen.CreateThread(function()
    ...
    -- Verificar si Susano está disponible con múltiples intentos
    local attempts = 0
    while Susano == nil and attempts < 5 do
        Wait(1000)
        attempts = attempts + 1
        print("^3[INFO]^7 Esperando Susano API... intento " .. attempts .. "/5")
    end
    
    if Susano == nil then
        print("^1[ERROR]^7 Susano API no detectada!")
        print("^3[INFO]^7 Este script requiere el executor Susano para funcionar")
        print("^3[INFO]^7 Descarga Susano desde: https://susano.re/")
        print("^3[TIP]^7 Asegúrate de tener Susano abierto y actualizado")
        return
    end
    
    print("^2[Sistema de Búsqueda]^7 Susano API detectada - UI cargada")
    ...
    while true do
        ...
        if showUI then
            ...
            if not success then
                print("^1[ERROR UI]^7 " .. tostring(err))
            end
            
            -- Detectar clicks en tabs (con protección)
            pcall(function()
...
-- Comandos
RegisterCommand('buscar', function()
    SearchAndOpenInventory()
end, false)

RegisterCommand('togglesearch', function()
    showUI = not showUI
    if showUI then
        ShowNotification("UI Mostrada", 2000)
    else
        ShowNotification("UI Oculta", 2000)
    end
end, false)

-- Teclas
RegisterKeyMapping('buscar', 'Buscar Jugador Cercano', 'keyboard', 'E')
RegisterKeyMapping('togglesearch', 'Mostrar/Ocultar UI', 'keyboard', 'F6')

print("^2[Sistema de Búsqueda]^7 Script cargado - UI con Susano Drawing API")
print("^2[Controles]^7 E: Buscar | F6: Ocultar/Mostrar | Click: Interactuar")
