...
-- Buscar Susano en diferentes ubicaciones
print("[Plaid] Iniciando script...")

local function FindSusano()
    -- Intentar diferentes ubicaciones comunes
    if Susano and type(Susano) == "table" then return Susano end
    if _G.Susano and type(_G.Susano) == "table" then return _G.Susano end
    if getgenv and getgenv().Susano and type(getgenv().Susano) == "table" then return getgenv().Susano end
    
    -- Buscar en globals comunes de executors
    for k, v in pairs(_G) do
        if type(v) == "table" and type(v.BeginFrame) == "function" and type(v.DrawText) == "function" then
            print("[Plaid] Encontrado API en: " .. tostring(k))
            return v
        end
    end
...
CreateThread(function()
    ...
    local SusanoAPI = FindSusano()
    
    if not SusanoAPI then
        print("[ERROR] No se encontró Susano API")
        print("[DEBUG] Variables globales disponibles:")
        for k, v in pairs(_G) do
            if type(v) == "table" and (string.find(string.lower(k), "sus") or string.find(string.lower(k), "draw")) then
                print("  - " .. k .. " (tipo: " .. type(v) .. ")")
            end
        end
        return
